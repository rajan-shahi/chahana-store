...
export default function Bags({ addToCart }) {
  ...
  const uniqueBrands = [
    ...new Set(ProductData.categories.bags.map((item) => item.brand)),
  ];
  const uniqueSizes = [
    ...new Set(
      ProductData.categories.bags.flatMap((item) => item.variants.sizes)
    ),
  ];
  const uniqueColors = [
    ...new Set(
      ProductData.categories.bags.flatMap((item) => item.variants.colors)
    ),
  ];
  const uniqueGenders = [
    ...new Set(ProductData.categories.bags.map((item) => item.category.gender)),
  ];
  const uniqueUses = [
    ...new Set(
      ProductData.categories.bags.flatMap((item) => item.category.use)
    ),
  ];
  const uniqueAgeGroups = [
    ...new Set(
      ProductData.categories.bags.map((item) => item.category.age_group)
    ),
  ];
  ...
  const filteredProducts = ProductData.categories.bags.filter((item) => {
    return (
      ...
      (filters.gender.length === 0 ||
        filters.gender.includes(item.category.gender)) &&
      (filters.use.length === 0 ||
        item.category.use.some((use) => filters.use.includes(use))) &&
      (filters.age.length === 0 ||
        filters.age.includes(item.category.age_group)) &&
